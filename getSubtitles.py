...
# Set to true if you don't want to synthesize the audio. For example, you already did that and are testing
...
# Set to true if you don't want to translate the subtitles. If so, ignore the following two variables
skipTranslation = parseBool(config['SETTINGS']['skip_translation'])
...
useFallbackGoogleTranslate = parseBool(
    cloudConfig['CLOUD']['use_fallback_google_translate'])
...
languageNums = batchConfig['SETTINGS']['enabled_languages'].replace(
    ' ', '').split(',')
originalVideoFile = os.path.abspath(
    batchConfig['SETTINGS']['original_video_file_path'].strip("\""))
...
for lineNum, line in enumerate(lines):
    ...
    if line.isdigit() and subtitleTimeLineRegex.match(lines[lineNum + 1]):
        ...
        subsDict[line] = {'start_ms': '', 'end_ms': '', 'duration_ms': '', 'text': '',
                          'break_until_next': '', 'srt_timestamps_line': lineWithTimestamps}
        ...
        processedTime1 = int(time1[0]) * 3600000 + int(time1[1]) * 60000 + int(time1[2].split(
            ',')[0]) * 1000 + int(time1[2].split(',')[1])  # / 1000 #Uncomment to turn into seconds
        processedTime2 = int(time2[0]) * 3600000 + int(time2[1]) * 60000 + int(time2[2].split(
            ',')[0]) * 1000 + int(time2[2].split(',')[1])  # / 1000 #Uncomment to turn into seconds
        ...
        if addBufferMilliseconds > 0:
            ...
            subsDict[line]['duration_ms_buffered'] = str(
                (processedTime2 - addBufferMilliseconds) - (processedTime1 + addBufferMilliseconds))
        else:
        ...
        if lineNum > 0:
            ...
            subsDict[str(int(line)-1)]['break_until_next'] = processedTime1 - \
                int(subsDict[str(int(line) - 1)]['end_ms'])
        else:
...
def combine_subtitles_advanced(inputDict, maxCharacters=200):
    ...
    def combine_single_pass(entryListLocal):
        ...
        while not reachedEndOfList:
            ...
            entryListLocal = calc_list_speaking_rates(
                entryListLocal, charRateGoal)
            ...
            for i, data in enumerate(priorityOrderedList):
                ...
                def combine_with_next():
                    entryListLocal[i]['text'] = entryListLocal[i]['text'] + \
                        ' ' + entryListLocal[i+1]['text']
                    entryListLocal[i]['translated_text'] = entryListLocal[i]['translated_text'] + \
                        ' ' + entryListLocal[i+1]['translated_text']
                    entryListLocal[i]['end_ms'] = entryListLocal[i+1]['end_ms']
                    entryListLocal[i]['end_ms_buffered'] = entryListLocal[i +
                                                                          1]['end_ms_buffered']
                    entryListLocal[i]['duration_ms'] = int(
                        entryListLocal[i+1]['end_ms']) - int(entryListLocal[i]['start_ms'])
                    entryListLocal[i]['duration_ms_buffered'] = int(
                        entryListLocal[i+1]['end_ms_buffered']) - int(entryListLocal[i]['start_ms_buffered'])
                    entryListLocal[i]['srt_timestamps_line'] = entryListLocal[i]['srt_timestamps_line'].split(
                        ' --> ')[0] + ' --> ' + entryListLocal[i+1]['srt_timestamps_line'].split(' --> ')[1]
                    del entryListLocal[i+1]

                def combine_with_prev():
                    entryListLocal[i-1]['text'] = entryListLocal[i -
                                                                 1]['text'] + ' ' + entryListLocal[i]['text']
                    entryListLocal[i-1]['translated_text'] = entryListLocal[i -
                                                                            1]['translated_text'] + ' ' + entryListLocal[i]['translated_text']
                    entryListLocal[i-1]['end_ms'] = entryListLocal[i]['end_ms']
                    entryListLocal[i -
                                   1]['end_ms_buffered'] = entryListLocal[i]['end_ms_buffered']
                    entryListLocal[i-1]['duration_ms'] = int(
                        entryListLocal[i]['end_ms']) - int(entryListLocal[i-1]['start_ms'])
                    entryListLocal[i-1]['duration_ms_buffered'] = int(
                        entryListLocal[i]['end_ms_buffered']) - int(entryListLocal[i-1]['start_ms_buffered'])
                    entryListLocal[i-1]['srt_timestamps_line'] = entryListLocal[i-1]['srt_timestamps_line'].split(
                        ' --> ')[0] + ' --> ' + entryListLocal[i]['srt_timestamps_line'].split(' --> ')[1]
                ...
                if data['char_rate'] > charRateGoal:
                    ...
                    # If only one is to be considered, then combine with that one
                        ...
                        print(
                            f"Error U: Should not reach this point! Current entry = {i}")
                    ...
                    # If only one is to be considered, then combine with that one
                        ...
                        print(
                            f"Error L: Should not reach this point! Index = {i}")
                        print(f"Current Entry Text = {data['text']}")
                        continue
...
def translate_dictionary(inputSubsDict, langDict, skipTranslation=False):
    ...
    if skipTranslation == False:
        if translateService == 'google' and codepoints > 27000 or translateService == 'deepl' and codepoints > 120000:
            ...
            chunkedTexts = [textToTranslate[x:x+chunkSize]
            ...
            for j, chunk in enumerate(chunkedTexts):
                ...
                if translateService == 'google':
                    ...
                    print(
                        f'[Google] Translating text group {j+1} of {len(chunkedTexts)}')
                    ...
                    translatedTexts = [response['translations'][i]['translatedText'] for i in range(
                    ...
                    for i in range(chunkSize):
                        ...
                        print(
                            f' Translated with Google: {key} of {len(inputSubsDict)}', end='\r')

                elif translateService == 'deepl':
                    print(
                        f'[DeepL] Translating text group {j+1} of {len(chunkedTexts)}')

                    # Send the request
                    result = auth.DEEPL_API.translate_text(
                        chunk, target_lang=targetLanguage, formality=formality)

                    # Extract the translated texts from the response
                    translatedTexts = [
                        result[i].text for i in range(len(result))]
                    ...
                    for i in range(chunkSize):
                        ...
                        # Print progress, ovwerwrite the same line
                        print(
                            f' Translated with DeepL: {key} of {len(inputSubsDict)}', end='\r')
                else:
                    print(
                        "Error: Invalid translate_service setting. Only 'google' and 'deepl' are supported.")
                    sys.exit()
            ...
            if translateService == 'google':
                ...
                translatedTexts = [response['translations'][i]['translatedText']
                ...
                for i, key in enumerate(inputSubsDict):
                    ...
                    print(
                        f' Translated: {key} of {len(inputSubsDict)}', end='\r')
                ...
                # Send the request
                result = auth.DEEPL_API.translate_text(
                    textToTranslate, target_lang=targetLanguage, formality=formality)
        ...
        for key in inputSubsDict:
            # Skips translating, such as for testing
            inputSubsDict[key]['translated_text'] = inputSubsDict[key]['text']
    print("                                                  ")

    combinedProcessedDict = combine_subtitles_advanced(
        inputSubsDict, combineMaxChars)
    ...
    if skipTranslation == False or debugMode == True:
        ...
        if debugMode:
            translatedSrtFileName = pathlib.Path(
                originalVideoFile).stem + f" - {lang} - {targetLanguage}.DEBUG.txt"
        else:
            translatedSrtFileName = pathlib.Path(
                originalVideoFile).stem + f" - {lang} - {targetLanguage}.srt"
        # Set path to save translated srt file
        translatedSrtFileName = os.path.join(
            outputFolder, translatedSrtFileName)
        ...
        with open(translatedSrtFileName, 'w', encoding='utf-8') as f:
            for key in combinedProcessedDict:
                f.write(str(key) + '\n')
                f.write(combinedProcessedDict[key]
                        ['srt_timestamps_line'] + '\n')
                f.write(combinedProcessedDict[key]['translated_text'] + '\n')
                if debugMode:
                    f.write(
                        f"DEBUG: duration_ms = {combinedProcessedDict[key]['duration_ms']}" + '\n')
                    f.write(
                        f"DEBUG: char_rate = {combinedProcessedDict[key]['char_rate']}" + '\n')
                    f.write(
                        f"DEBUG: start_ms = {combinedProcessedDict[key]['start_ms']}" + '\n')
                    f.write(
                        f"DEBUG: end_ms = {combinedProcessedDict[key]['end_ms']}" + '\n')
                    f.write(
                        f"DEBUG: start_ms_buffered = {combinedProcessedDict[key]['start_ms_buffered']}" + '\n')
                    f.write(
                        f"DEBUG: end_ms_buffered = {combinedProcessedDict[key]['end_ms_buffered']}" + '\n')
                f.write('\n')
...
def set_translation_info(languageBatchDict):
    ...
    if preferredTranslateService == 'deepl':
        ...
        supportedLanguagesList = list(
            map(lambda x: str(x.code).upper(), langSupportResponse))
...
def process_language(langData):
    ...
    # Translate
    individualLanguageSubsDict = translate_dictionary(
        individualLanguageSubsDict, langDict, skipTranslation=skipTranslation)
    ...
    if batchSynthesize == True and tts_service == 'azure':
        individualLanguageSubsDict = TTS.synthesize_dictionary_batch(
            individualLanguageSubsDict, langDict, skipSynthesize=skipSynthesize)
    else:
    ...
    individualLanguageSubsDict = audio_builder.build_audio(
        individualLanguageSubsDict, langDict, totalAudioLength, twoPassVoiceSynth)
